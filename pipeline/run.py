...
工具（非流水线阶段）:
    ...
    push_batch.sh  大批量 PDF 分批推送 GitHub（单次 push 超 ~100MB 易被掐断）
...
STAGES = {
    ...
    "ingest-dir":("s3_ingest_dir","S3 批量归档整目录"),
...
def status():
    ...
    # S8 交付：不分批次，一套判据出一套交付物
    try:
        sys.path.insert(0, HERE)
        import s8_build
        rows, drop = s8_build.select(("S", "A", "B"))
        tc = collections.Counter(r["tier"] for r in rows)
        print(f"{mark(len(rows))} S8 可交付          {len(rows):>7} 篇  "
              f"S{tc['S']} A{tc['A']} B{tc['B']}   (排除 " +
              " ".join(f"{k}{v}" for k, v in sorted(drop.items())) + ")")
        for d in sorted(glob.glob(f"{C.ROOT}/*/信息抽取*.xlsx")):
            rel = os.path.relpath(d, C.ROOT)
            pdir = os.path.join(os.path.dirname(d), "pdfs")
            alt = [p for p in glob.glob(os.path.join(os.path.dirname(d), "pdfs_*"))
                   if os.path.isdir(p)]          # 别把 pdfs_batch2.zip 当目录
            pdir = pdir if os.path.isdir(pdir) else (alt[0] if alt else "")
            npdf = len([f for f in os.listdir(pdir) if f.lower().endswith(".pdf")]) if pdir else 0
            print(f"      已出交付: {rel:<34s} PDF {npdf}")
    except Exception as e:
        print(f"· S8 可交付          （无法计算: {e}）")
