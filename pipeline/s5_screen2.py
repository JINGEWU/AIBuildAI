...
def load_digest():
    ...
    # digest 是 append-only：同一 DOI 重抽会追加新记录，必须后到覆盖先到，
    # 否则 XML/arXiv 版升级成官方 PDF 后，判读读到的还是旧证据。
    byd = {}
    for r in m.read_jsonl(f"{C.DATA}/fulltext_digest.jsonl"):
        d = m.norm_doi(r.get("doi"))
        if d:
            byd[d] = r
    rows = list(byd.values())
    def key(r):
...
# ───────────────────────── dump ─────────────────────────
def dump(batch, size, out, pool=None, redo=False):
    rows = load_digest()
    V = load_v()
    if pool:
        keep = {m.norm_doi(x) for x in json.load(open(pool, encoding="utf-8"))}
        rows = [r for r in rows if m.norm_doi(r["doi"]) in keep]
        m.log(f"限定判读池 {len(keep)} 篇，digest 命中 {len(rows)}")
    todo = rows if redo else [r for r in rows if m.norm_doi(r["doi"]) not in V]
...
def main(argv):
    ...
    ap.add_argument("--pool", help="只判这个 json 里的 DOI 列表")
    ap.add_argument("--redo", action="store_true", help="连已判过的一起重判（证据变了时用）")
    a = ap.parse_args(argv)
    if a.step == "check":
        check()
    elif a.step == "dump":
        dump(a.batch, a.size, a.out, a.pool, a.redo)
    elif a.step == "save":
        if not a.file:
            m.die("save 需要 --file")
        save(a.file)
